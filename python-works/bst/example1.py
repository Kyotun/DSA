class BinarySearchTreeNode:
    ...
    def delete_right_min(self, val):
        if val < self.data:
            if self.left:
                self.left = self.left.delete_right_min(val)
        elif val > self.data:
            if self.right:
                self.right = self.right.delete_right_min(val)
        else:
...
if __name__ == '__main__':
    ...
    numbers_tree.delete_right_min(20)
    print("After deleting 20: ", numbers_tree.in_order_traversal())  # this should print [1, 4, 9, 17, 18, 23, 34]

    numbers_tree = build_tree([17, 4, 1, 20, 9, 23, 18, 34])
    numbers_tree.delete_left_max(9)
    print("After deleting 9: ", numbers_tree.in_order_traversal())  # this should print [1, 4, 17, 18, 20, 23, 34]

    numbers_tree = build_tree([17, 4, 1, 20, 9, 23, 18, 34])
    numbers_tree.delete_right_min(17)
